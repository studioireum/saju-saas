# -*- coding: utf-8 -*-
import os, urllib.request, json
from flask import Flask, request, jsonify, send_from_directory
from flask_cors import CORS
from lunar_python import Solar

app = Flask(__name__, static_folder='.')
CORS(app)

CG=['甲','乙','丙','丁','戊','己','庚','辛','壬','癸']
JJ=['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥']
CO=['木','木','火','火','土','土','金','金','水','水']
JO=['水','土','木','木','土','火','火','土','金','金','土','水']
CU=['양','음','양','음','양','음','양','음','양','음']
SAENG={'木':'火','火':'土','土':'金','金':'水','水':'木'}
KUK={'木':'土','土':'水','水':'火','火':'金','金':'木'}
JJMAP={'子':'癸','丑':'己','寅':'甲','卯':'乙','辰':'戊','巳':'丙','午':'丁','未':'己','申':'庚','酉':'辛','戌':'戊','亥':'壬'}
JDAY=[4,6,5,6,6,7,7,8,8,7,7,6]

def sipseong(ig,tg):
    io=CO[CG.index(ig)];to=CO[CG.index(tg)]
    iu=CU[CG.index(ig)];tu=CU[CG.index(tg)];s=iu==tu
    if io==to: return '비견' if s else '겁재'
    if SAENG[to]==io: return '정인' if s else '편인'
    if KUK[to]==io: return '정관' if not s else '편관'
    if SAENG[io]==to: return '식신' if s else '상관'
    if KUK[io]==to: return '정재' if not s else '편재'
    return '?'

def calc_dws(y,m,d,fwd):
    from datetime import date
    birth=date(y,m,d);jday=JDAY[m-1]
    if fwd:
        if d<jday: target=date(y,m,jday)
        else:
            nm=m+1 if m<12 else 1;ny=y if m<12 else y+1
            target=date(ny,nm,JDAY[nm-1])
        diff=(target-birth).days
    else:
        if d>=jday: target=date(y,m,jday)
        else:
            pm=m-1 if m>1 else 12;py=y if m>1 else y-1
            target=date(py,pm,JDAY[pm-1])
        diff=(birth-target).days
    return max(1,round(diff/3))

def calc_saju(y,m,d,h,gender):
    sol=Solar.fromYmdHms(y,m,d,h,0,0);lun=sol.getLunar()
    def split(gj): return gj[0],gj[1]
    ycg,yjj=split(lun.getYearInGanZhi())
    mcg,mjj=split(lun.getMonthInGanZhi())
    dcg,djj=split(lun.getDayInGanZhi())
    hcg,hjj=split(lun.getTimeInGanZhi())
    P=[{'cg':ycg,'jj':yjj},{'cg':mcg,'jj':mjj},{'cg':dcg,'jj':djj},{'cg':hcg,'jj':hjj}]
    OH={'木':0,'火':0,'土':0,'金':0,'水':0}
    for p in P:
        OH[CO[CG.index(p['cg'])]]+=1
        OH[JO[JJ.index(p['jj'])]]+=1
    NK=['년','월','일','시'];SS={}
    for i,p in enumerate(P):
        if NK[i]!='일': SS[NK[i]+'간']=sipseong(dcg,p['cg'])
        SS[NK[i]+'지']=sipseong(dcg,JJMAP[p['jj']])
    yum=CU[CG.index(ycg)]
    fwd=(gender=='남' and yum=='양') or (gender=='여' and yum=='음')
    dws=calc_dws(y,m,d,fwd)
    mi,mji=CG.index(mcg),JJ.index(mjj);DW=[]
    for i in range(1,9):
        ci=((mi+(i if fwd else -i))%10+10)%10
        ji=((mji+(i if fwd else -i))%12+12)%12
        DW.append({'cg':CG[ci],'jj':JJ[ji],'age':dws+(i-1)*10,'yr':y+dws+(i-1)*10})
    return {'P':P,'ilgan':dcg,'OH':OH,'SS':SS,'DW':DW,'dws':dws}

SYSTEM="""당신은 맹파(盲派) 물상론을 중심으로 정통 명리학을 통합 해석하는 전문가입니다. 천간지지를 살아있는 자연물로 읽습니다: 甲木=큰소나무, 乙木=화초덩굴, 丙火=태양, 丁火=촛불별빛, 戊土=산제방, 己土=논밭, 庚金=도끼바위, 辛金=보석칼날, 壬水=바다강, 癸水=빗물이슬.

해석 원칙: 1)결론부터 말하라 근거는 짧게 뒤에 붙인다 2)"일 수 있습니다" "할지도 모릅니다" "경우에 따라" 같은 얼버무림 금지 3)장점과 약점을 회피하지 말고 둘 다 분명히 지적하라 4)위로하거나 돌려 말하지 말라 상담자는 정확한 진단을 원한다 5)불필요한 수식어 제거 한 문장에 하나의 핵심만.

해석 순서: 1)일간 물상과 전체 풍경화 2)오행 결핍·과다의 현실적 의미 3)현재 대운의 변화 방향 4)직업·재물·관계·건강의 핵심 지점 5)지금 내려야 할 결단.

규칙: 마크다운 금지, 산문체 700자 내외, 제공된 원국 그대로 사용, 재계산 금지. 명료하고 단단한 문장으로 쓸 것."""

def call_gemini(messages):
    key=os.environ.get('GEMINI_API_KEY','').strip()
    if not key: return None,'GEMINI_API_KEY 없음'
    prompt=SYSTEM+'\n\n'
    for msg in messages:
        role='사용자' if msg['role']=='user' else '어시스턴트'
        prompt+=f'{role}: {msg["content"]}\n\n'
    prompt+='어시스턴트:'
    url=f'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={key}'
    data=json.dumps({'contents':[{'parts':[{'text':prompt}]}],'generationConfig':{''maxOutputTokens':8000,'thinkingConfig':{'thinkingBudget':0}}).encode()
    req=urllib.request.Request(url,data=data,headers={'Content-Type':'application/json'})
    try:
        resp=urllib.request.urlopen(req,timeout=60)
        result=json.loads(resp.read())
        return result['candidates'][0]['content']['parts'][0]['text'],None
    except urllib.error.HTTPError as e:
        return None,f'Gemini 오류: {e.read().decode()[:200]}'
    except Exception as e:
        return None,str(e)

@app.route('/')
def index(): return send_from_directory('.','saju_final.html')

@app.route('/api/saju',methods=['POST'])
def saju():
    data=request.get_json(silent=True)
    if not data: return jsonify({'error':'입력 없음'}),400
    try:
        y=int(data['year']);m=int(data['month']);d=int(data['day'])
        h=int(data['hour']);g=data.get('gender','남')
        return jsonify(calc_saju(y,m,d,h,g))
    except Exception as e:
        return jsonify({'error':str(e)}),500

@app.route('/api/analyze',methods=['POST'])
def analyze():
    data=request.get_json(silent=True)
    if not data or not data.get('messages'):
        return jsonify({'error':'메시지 없음'}),400
    text,err=call_gemini(data['messages'])
    if err: return jsonify({'error':err}),500
    return jsonify({'text':text})

@app.route('/api/health')
def health():
    key=os.environ.get('GEMINI_API_KEY','').strip()
    return jsonify({'status':'ok','api_key':'set' if key else 'missing'})

if __name__=='__main__':
    port=int(os.environ.get('PORT',5000))
    app.run(debug=False,port=port,host='0.0.0.0')
